...
async function main() {
    ...
    console.log('=== FIXING HERO TO DISPLAY UPLOADED IMAGES ===\n');

    const newHeroSection = `{% comment %}
  ISKCON Hero Banner Section - displays uploaded deity images
{% endcomment %}
...
<section class="iskcon-hero">
  ...
  <div class="iskcon-images">
    <div class="iskcon-image-wrapper">
      <div class="iskcon-image-container {% unless section.settings.deity_image_1 %}deity-nitai-gaur{% endunless %}">
        {% if section.settings.deity_image_1 %}
          <img src="{{ section.settings.deity_image_1 | image_url: width: 320 }}"
               alt="Sri Sri Nitai Gauranga"
               loading="eager">
        {% else %}
          <span class="deity-placeholder">🙏</span>
        {% endif %}
      </div>
    ...
    <div class="iskcon-image-wrapper">
      <div class="iskcon-image-container {% unless section.settings.deity_image_2 %}deity-radha-krishna{% endunless %}">
        {% if section.settings.deity_image_2 %}
          <img src="{{ section.settings.deity_image_2 | image_url: width: 320 }}"
    ...
    <div class="iskcon-image-wrapper">
      <div class="iskcon-image-container {% unless section.settings.deity_image_3 %}deity-sita-rama{% endunless %}">
        {% if section.settings.deity_image_3 %}
          <img src="{{ section.settings.deity_image_3 | image_url: width: 320 }}"
...
{% endschema %}`;
    ...
    if (result.status === 200) {
        ...
        console.log('\nThe section now:');
        console.log('  - Displays uploaded images using {{ section.settings.deity_image_X }}');
        console.log('  - Falls back to gradient + emoji if no image uploaded');
        console.log('\nPlease save the theme customizer and refresh the page.');
    } else {
